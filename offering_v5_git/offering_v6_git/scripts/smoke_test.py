...
def custom_backtest_threshold_fixture_test() -> None:
    repo = BacktestRepository(ROOT / "data")
    custom = repo.custom_threshold_view(1.2)
    official = repo.load_summary("1.2", pretty=True)
    summary = custom.get("summary", pd.DataFrame())
    assert not summary.empty
    merged = summary.merge(
        official,
        on=["strategy_name", "term", "entry_mode", "hold_days_after_entry"],
        suffixes=("_custom", "_official"),
        how="inner",
    )
    assert len(merged) == len(official)
    for col in ["trades", "win_rate", "avg_ret", "median_ret", "sum_ret", "compound_ret", "min_ret", "max_ret"]:
        diff = (pd.to_numeric(merged[f"{col}_custom"], errors="coerce") - pd.to_numeric(merged[f"{col}_official"], errors="coerce")).abs().max()
        assert float(diff or 0.0) == 0.0, f"mismatch in {col}: {diff}"


def calendar_timeline_fixture_test() -> None:
    repo = IPORepository(ROOT / "data")
    issues = pd.DataFrame(
        [
            {
                "name": "테스트공모",
        ...
        [
            {
                ...
                "symbol": "123456",
                "market": "코스닥",
                "unlock_date": "2026-05-10",
                "term": "1M",
            }
        ]
    )
    timeline = repo.build_event_timeline(issues, unlocks)
    assert len(timeline) == 4
    assert set(timeline["event_type"].tolist()) == {"청약시작", "청약종료", "상장", "보호예수해제"}
    assert timeline["date"].is_monotonic_increasing



def turnover_strategy_service_fixture_test() -> None:
    from src.services.turnover_strategy_service import TurnoverStrategyParams, TurnoverStrategyService

    def history_provider(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        dates = pd.bdate_range(pd.Timestamp(start_date), pd.Timestamp(end_date))
        if len(dates) == 0:
            return pd.DataFrame(columns=["date", "open", "high", "low", "close", "volume"])
        base = 10000 + int(str(symbol)[-2:])
        return pd.DataFrame(
            {
                "date": dates,
                "open": [base + idx for idx, _ in enumerate(dates)],
                "high": [base + idx + 5 for idx, _ in enumerate(dates)],
                "low": [base + idx - 5 for idx, _ in enumerate(dates)],
                "close": [base + idx + 2 for idx, _ in enumerate(dates)],
                "volume": [100000] * len(dates),
            }
        )

    service = TurnoverStrategyService(ROOT / "data", history_provider=history_provider)
    params = TurnoverStrategyParams(
        multiples=(1.0,),
        price_filters=("reclaim_open_or_vwap",),
        max_days_after=10,
        unlock_end_date="2026-03-28",
        max_events=4,
        hold_days_by_term={"15D": 2, "1M": 3, "6M": 4},
    )
    result = service.run_workspace_research(ROOT / "data" / "sample_unified_lab_workspace", params)
    assert not result["signals"].empty
    assert not result["trades"].empty
    assert not result["summary"].empty


def shorts_service_fixture_test() -> None:
    from src.services.shorts_service import ShortsStudioService
    from src.services.scoring import IPOScorer

    hub = IPODataHub(ROOT / "data")
    ...
    studio = ShortsStudioService(ROOT / "data")
    payload = studio.build_daily_payload(bundle, issues, pd.Timestamp("2026-03-26"), window_days=7, source_label="샘플")
    with TemporaryDirectory() as tmpdir:
        result = studio.generate_assets(payload, Path(tmpdir), title="테스트 쇼츠", create_video=False, create_zip=True)
        assert len(result.get("slides", [])) >= 3
        assert Path(result["zip_path"]).exists()
        assert Path(result["captions_path"]).exists()


def source_normalization_fixture_test() -> None:
    from src.utils import standardize_issue_frame

    raw = pd.DataFrame([
        {"name": "테스트IPO", "source": 38, "market": "코스닥", "subscription_start": "2026-04-01", "subscription_end": "2026-04-02", "underwriters": "한국투자증권"},
        {"name": "다른IPO", "source": "strategy-overlay", "market": "코스피", "listing_date": "2026-04-10"},
    ])
    out = standardize_issue_frame(raw)
    values = out["source"].dropna().tolist()
    assert "38" in values
    assert any(isinstance(value, str) for value in values)


def unified_empty_csv_fixture_test() -> None:
    from src.services.unified_lab_bridge import UnifiedLabBridgeService

    with TemporaryDirectory() as tmpdir:
        workspace = Path(tmpdir) / "workspace"
        signal_dir = workspace / "signal_out"
        unlock_dir = workspace / "unlock_out"
        signal_dir.mkdir(parents=True, exist_ok=True)
        unlock_dir.mkdir(parents=True, exist_ok=True)
        (unlock_dir / "unlock_events_backtest_input.csv").write_text("name,unlock_date,term\n테스트IPO,2026-04-10,1M\n", encoding="utf-8")
        (signal_dir / "turnover_signals.csv").write_text("\ufeff\n", encoding="utf-8")
        service = UnifiedLabBridgeService(ROOT / "data")
        bundle = service.load_bundle(workspace)
        assert bundle.signals.empty
...
def main() -> None:
    ...
    turnover_strategy_service_fixture_test()
    shorts_service_fixture_test()
    source_normalization_fixture_test()
    unified_empty_csv_fixture_test()
    print("SMOKE TEST OK")
