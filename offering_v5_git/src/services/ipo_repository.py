...
class IPORepository:
    ...
    def build_event_timeline(self, issues: pd.DataFrame, unlocks: pd.DataFrame) -> pd.DataFrame:
        rows: list[dict[str, Any]] = []
        work_issues = standardize_issue_frame(issues.copy())
        work_unlocks = parse_date_columns(unlocks.copy(), ["unlock_date"]) if isinstance(unlocks, pd.DataFrame) else pd.DataFrame()

        if not work_issues.empty:
            for _, row in work_issues.iterrows():
        ...
        if not work_unlocks.empty:
            for _, row in work_unlocks.iterrows():
                if pd.isna(row.get("unlock_date")):
                    continue
                rows.append(
                    {
                        "date": row.get("unlock_date"),
                        "event_type": "보호예수해제",
                        "name": row.get("name"),
                        "symbol": row.get("symbol"),
                        "market": row.get("market"),
    ...
    def build_timeline(self, issues: pd.DataFrame, unlocks: pd.DataFrame, today: pd.Timestamp, window_days: int = 30) -> pd.DataFrame:
        timeline = self.build_event_timeline(issues, unlocks)
        if timeline.empty:
            return timeline
        today = pd.Timestamp(today).normalize()
        end = today + pd.Timedelta(days=window_days)
        mask = timeline["date"].between(today, end, inclusive="both")
        return timeline.loc[mask].reset_index(drop=True)
