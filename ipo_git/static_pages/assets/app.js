...
function renderDashboard() {
  ...
  const quotes = (feed.marketQuotes || []).slice(0, 9);
  el('#market-grid').innerHTML = quotes.length ? quotes.map((quote) => {
    ...
    return `
      <div class="quote-card">
        ...
        <div class="table-sub">${escapeHtml(quote.group || '')} · ${escapeHtml(formatDateFull(quote.asOf))}</div>
      </div>
    `;
...
function renderSubscriptions() {
  ...
  el('#subscription-meta').textContent = `${items.length}건`;
  el('#subscription-list').innerHTML = items.length ? items.map(renderIssueCard).join('') : emptyState('조건에 맞는 청약 종목이 없습니다.');
}
...
function renderUnlocks() {
  ...
  el('#unlock-meta').textContent = `${rows.length}건`;
  el('#unlock-table tbody').innerHTML = rows.slice(0, 400).map((row) => `
    <tr>
...
function renderExplorer() {
  ...
  el('#explorer-meta').textContent = `${items.length}건`;
  el('#explorer-table tbody').innerHTML = items.slice(0, 500).map((item) => {
    const returnClass = item.returnPct === null ? '' : item.returnPct >= 0 ? 'good' : 'bad';
    return `
      <tr>
...
function renderDataHealth() {
  ...
  const cacheEntries = Object.entries(cacheInventory);
