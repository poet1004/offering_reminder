...
def _prepare_issue_frame(df: pd.DataFrame, *, preserve_extra: bool = False) -> pd.DataFrame:
    ...
    if not out.empty:
        out["stage"] = out.apply(
            lambda row: infer_issue_stage(
                row.get("subscription_start"),
                row.get("subscription_end"),
                row.get("listing_date"),
                fallback=row.get("stage"),
            ),
            axis=1,
        )
...
def issue_recency_sort(df: pd.DataFrame, *, today: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    work = _prepare_issue_frame(df.copy(), preserve_extra=True)
    if work.empty:
        return work
